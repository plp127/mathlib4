...
theorem zmodAddEquivOfGenerator_apply_intCast [AddGroup G]
    ...
    (i : ℤ) : zmodAddEquivOfGenerator hg hn i = i • g := by
...
@[simp]
theorem zmodAddEquivOfGenerator_symm_apply_zsmul [AddGroup G]
    {g : G} (hg : ∀ x, x ∈ zmultiples g) {n : ℕ} (hn : Nat.card G = n)
    (i : ℤ) : (zmodAddEquivOfGenerator hg hn).symm (i • g) = i := by
  rw [AddEquiv.symm_apply_eq, zmodAddEquivOfGenerator_apply_intCast]

@[simp]
theorem zmodAddEquivOfGenerator_apply_one [AddGroup G]
    {g : G} (hg : ∀ x, x ∈ zmultiples g) {n : ℕ} (hn : Nat.card G = n) :
    zmodAddEquivOfGenerator hg hn 1 = g := by
  simpa using zmodAddEquivOfGenerator_apply_intCast hg hn 1

@[simp]
theorem zmodAddEquivOfGenerator_symm_apply_generator [AddGroup G]
    {g : G} (hg : ∀ x, x ∈ zmultiples g) {n : ℕ} (hn : Nat.card G = n) :
...
and instead picks one out with the axiom of choice. -/
noncomputable def zmodMulEquivOfGenerator [Group G] {g : G} (hg : ∀ x, x ∈ zpowers g)
    {n : ℕ} (hn : Nat.card G = n) : Multiplicative (ZMod n) ≃* G :=
  AddEquiv.toMultiplicative <| zmodAddEquivOfGenerator (G := Additive G) hg hn

@[simp]
theorem zmodMulEquivOfGenerator_apply_ofAdd_intCast [Group G]
...
@[simp]
theorem zmodMulEquivOfGenerator_symm_apply_zpow [Group G]
    {g : G} (hg : ∀ x, x ∈ zpowers g) {n : ℕ} (hn : Nat.card G = n)
    (i : ℤ) : (zmodMulEquivOfGenerator hg hn).symm (g ^ i) =
      Multiplicative.ofAdd (i : ZMod n) :=
  zmodAddEquivOfGenerator_symm_apply_zsmul (G := Additive G) hg hn i

@[simp]
theorem zmodMulEquivOfGenerator_apply_ofAdd_one [Group G]
    {g : G} (hg : ∀ x, x ∈ zpowers g) {n : ℕ} (hn : Nat.card G = n) :
    zmodMulEquivOfGenerator hg hn (Multiplicative.ofAdd 1) = g :=
  zmodAddEquivOfGenerator_apply_one (G := Additive G) hg hn

@[simp]
theorem zmodMulEquivOfGenerator_symm_apply_generator [Group G]
    {g : G} (hg : ∀ x, x ∈ zpowers g) {n : ℕ} (hn : Nat.card G = n) :
