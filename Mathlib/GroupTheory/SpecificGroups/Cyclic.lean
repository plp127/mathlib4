...
noncomputable def zmodAddCyclicAddEquiv [AddGroup G] (h : IsAddCyclic G) :
    ZMod (Nat.card G) ≃+ G :=
  zmodAddEquivOfGeneratorOfCardEq h.exists_generator.choose_spec rfl
...
See `zmodAddEquivOfGeneratorOfCardEq` for a version which takes an explicit generator. -/
...
See `zmodMulEquivOfGeneratorOfCardEq` for a version which takes an explicit generator. -/
