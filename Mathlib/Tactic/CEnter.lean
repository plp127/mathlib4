...
syntax (name := cEnter) "cEnter" cLoc : conv

open Meta Elab Tactic Conv in
@[tactic cEnter]
def evalCEnter : Tactic := fun stx => do
  let pathStx := stx[1]
  let path? := parseCLoc {raw := pathStx}
  if let some path := path? then
    Term.withSynthesize <| withMainContext do
      let rec traverse (mvarId : MVarId) (lhs rhs : Expr) (path : Path) : TacticM MVarId := do
        let lhs ← instantiateMVars lhs
        let rhs ← instantiateMVars rhs
        match path with
        | .node => return mvarId
        | .proj next =>
          let .proj n idx e := lhs.consumeMData
            | throwError "invalid 'proj', expected projection{indentExpr lhs}"
          let type ← inferType e
          let t ← inferType lhs
          let u₁ ← getLevel type
          let u₂ ← getLevel t
          let cong : Expr := .lam .anonymous type (.proj n idx (.bvar 0)) .default
          let (g, new) ← mkConvGoalFor e
          resolveRhs "proj" rhs (.proj n idx g)
          let prf := mkApp6 (.const ``congrArg [u₁, u₂]) type t e g cong new
          mvarId.assign prf
          traverse new.mvarId! e g next
        | .fun n next =>
          let mut lhs := lhs
          let mut rhs := rhs
          let mut mvarId := mvarId
          for _ in [:n] do
            let .app f a := lhs.consumeMData
              | throwError "invalid 'fun', expected application{indentExpr lhs}"
            let (g, mvarNew) ← mkConvGoalFor f
            mvarId.assign (← mkCongrFun mvarNew a)
            resolveRhs "fun" rhs (.app g a)
            mvarId := mvarNew.mvarId!
            lhs := f
            rhs := g
          return mvarId
        | .type next
        | .body next
        | .value next
        | .arg n all next => throwError m!"not implemented"
      let mvarId ← getMainGoal
      let (lhs, rhs) ← getLhsRhs
      let new ← traverse mvarId lhs rhs path
      replaceMainGoal [new]
  else
    throwError "invalid path{indentD pathStx}"
where
  resolveRhs (name : String) (rhs rhs' : Expr) : MetaM Unit := do
    unless (← isDefEqGuarded rhs rhs') do
      throwError m!"invalid '{name}', failed to resolve{indentExpr rhs}\n=?={indentExpr rhs'}"
